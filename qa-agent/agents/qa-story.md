...
6. Build a self-validation block **`_validation`** = `{ checklist: [{item,pass}], selfConfident, notes }`. The `checklist` must include at least these items, each with a boolean `pass`:
   ...
   Set `selfConfident` to a number/boolean reflecting your confidence in the normalization, and `notes` to any caveats (e.g. ACs were inferred, description was sparse, ambiguous wording).
