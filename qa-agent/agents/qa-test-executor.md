...
1. Before running any case, obtain the credential values exactly once, using **Bash for this purpose only**, resolving each of `config.app.login.usernameEnv` and `passwordEnv` **in this priority order**:
...
2. If neither the env var nor the `.qa-secrets`/`.env` file provides a non-empty value, do NOT fabricate a value or attempt a login: mark every case that depends on being logged in as **`blocked`** with `reason: "credential <NAME> not set (no env var and no .qa-secrets entry)"` (substituting the actual variable name), and continue to the rest of the run per the isolation rule below.
