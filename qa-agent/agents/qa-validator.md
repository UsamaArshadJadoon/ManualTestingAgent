...
- **`story`** — inputs: `run-context.json` (for the issue `key`) + a fresh `mcp__claude_ai_Atlassian__getJiraIssue` re-fetch. Output checked: `story.json`.
  ...
  - **design-dependent AC flagged** — read the raw description in your re-fetch. Every AC defined by reference to an image ("as per the attached wireframe", "matches the design", "as shown in the mockup") must carry `designDependent: true`, and each image the story attaches must appear in `designAttachments` with its filename, URL and the AC ids that depend on it. An AC that is visually defined but flagged `false` is a gap: it lets the pipeline claim coverage it cannot deliver. Emitting `designAttachments: []` for a story with genuinely no design reference is correct, not a gap.
...
- **`test-writer`** — inputs: `story.json`. Output checked: `test-cases.json`.
  ...
  - **criticality assigned and discriminating** — every case has a `criticality` of `blocker`, `major` or `minor`. Also check the *distribution*: if nearly every case is `blocker`, the field has been filled in without being used, and a plan whose cases are all equally critical cannot be triaged or time-boxed. Treat a degenerate spread as a gap and say which cases look mis-graded. Judge the grading too, not just its presence: a negative case proving a mandatory validation belongs at `blocker`, and a happy-path case for a cosmetic label belongs at `minor` — flag any case whose criticality looks copied from its `type`.
...
- **`bug-logger-create`** — inputs: `bugs-proposed.json` + the orchestrator-supplied approved-refs list. Output checked: `bugs-created.json`.
  ...
  - **no duplicate creation** (each `ref` appears exactly once in `created`, and no two entries share a `key`). Phase B can legitimately be re-entered — by a fix-retry of this very stage, by `--resume`, or after a partial failure — so it is required to skip refs already recorded and carry their entries forward. Two entries for one `ref`, or one `ref` filed under two keys, means that guard did not hold and duplicate issues now exist on the board. Report it as a gap **and say plainly in `gaps` that duplicate Jira issues were created and need deleting**, since re-running this stage again will not remove them.
  ...
  - consistent markdown descriptions (nothing in the run's reported output indicates a bug was filed with Jira **wiki** markup — `h3.`, `{{code}}`, `{code}`, or `*bold*` as a heading/emphasis syntax — instead of markdown. The MCP renders markdown, so wiki markup lands in the ticket as literal characters. Flag it as a gap if the producing agent reports having used wiki markup for any issue, or reports different syntaxes across issues in the same run: two differently-formatted tickets from one run is a real defect in the output, even though the content is present.)
...
- **`reviewer`** — inputs: `story.json`, `test-cases.json`, `gap-report.json`, `results.json`, `run-context.json`, and `bugs-proposed.json` (optional — read it if present; its absence is not a terminal failure, see below). Output checked: `review.json`. This is the one stage where you must fully recompute the reviewer's numbers yourself from the upstream files, not just read `review.json`'s own claims — a self-consistency check against `review.json` alone would not catch a wrong number that happens to be internally consistent.
  ...
  - **design-dependent AC not silently counted as fully verified** — if `story.json` has any AC with `designDependent: true` and the run folder has no `wireframe-spec.md`, those criteria were tested for structure only, because the wireframe never reached the pipeline. `review.json`'s `rationale` must say so and name them. A verdict that reports them as fully covered is overstating what the run proved, and is a gap even when every arithmetic check above passes. This is the same failure mode as covered-but-unsatisfied: a number that is internally consistent and still misleading.
