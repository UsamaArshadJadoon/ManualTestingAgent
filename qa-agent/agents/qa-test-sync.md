...
2. Find the folder whose `name` starts with the story key (e.g. `PROJ-123`). Use its `ID` as the target folder.
...
Return a one-line summary, e.g. `aio-sync.json written: 26/26 cases created in AIO folder PROJ-123 (linked to PROJ-123)`.
