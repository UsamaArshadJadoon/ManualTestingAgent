...
1. Dispatch **`qa-validator`** via the Task tool, passing the run-folder path, the **`stage`** name for the stage just completed, and the fix-retry **`iteration`** number (start at `0` for the first validation of a stage in this run). The validator stage names are exactly: `story`, `test-writer`, `gap-analyzer`, `test-executor`, `verification`, `bug-logger-propose`, `bug-logger-create`, `reviewer`. It writes `validation/<stage>.json` with `pass` and concrete `gaps`.
...
1. **`qa-story`** → writes `story.json` (normalized summary, description, atomic `acceptanceCriteria`, `designAttachments`, components, status). **→ validate** stage `story` (the validator independently re-fetches the Jira issue to confirm no AC was dropped).

   **1a. Design-input gate — do this before the writer runs.** Read `story.json`. If any AC has `designDependent: true` **and** no `wireframe-spec.md` exists in the run folder, those criteria are defined by an image nobody in the pipeline can see: `qa-story` has no capability to download attachment bytes. Left alone, the writer emits structure-only cases, the reviewer counts the AC as covered, and the final report publishes a coverage figure the run never earned.

   Do not let that pass silently. Use **AskUserQuestion**, naming the affected AC ids and the filenames from `designAttachments`, and offer:

   - **Supply the designs** — the user saves the images into the run folder, or pastes/attaches them to the conversation so you can transcribe them into `wireframe-spec.md` (a plain-text contract: which controls exist, how they are composed, the intended layout; mark anything uncertain as `[UNCERTAIN]`). Then continue. This is the recommended path and the only one that gives real coverage of those AC.
   - **Proceed structurally** — the user explicitly accepts that those AC are covered by structure only. Record the decision by writing `wireframe-spec.md` containing just that acknowledgement and the affected AC ids, so every downstream agent and the final report can see the coverage was accepted as partial rather than assumed complete.

   Whichever is chosen, **no pixel, layout, spacing or colour defect may be filed from a run without a real `wireframe-spec.md`** — an assertion about a design nobody has seen manufactures defects against correct behaviour. If the story has no design-dependent AC, skip this gate entirely and say nothing about it.
...
4. **Test-plan approval gate.** Before ANY browser execution, present the planned test cases to the user: read `test-cases.json` and show a concise numbered list (id, title, type, `criticality`, `linkedAC`) plus the coverage status from `gap-report.json`. **Group or sort the list by `criticality` (blocker → major → minor) and show the count of each**, so the user can see at a glance what the plan treats as essential. On a large plan this is the difference between an approvable summary and an unreadable wall: it also lets the user approve a subset by criticality ("run the blockers now") rather than having to read every case. Use **AskUserQuestion** and **wait for the user to say `go`** (approve) before proceeding. If the user wants changes, relay them to `qa-test-writer` (re-validate, and re-run gap analysis) before presenting the plan again. Do not dispatch the executor until the user has approved the plan.
